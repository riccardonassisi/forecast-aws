...
const calcolaImporti = async (user, result) => {
    ...
    let prev = Number(result.PrevisioneTotale);
    let flatf = Number(user.FlatFee);
    let disc = Number(user.Discount);
    let mark = Number(user.Markup);
    let cong = 0;
    let imp = 0;
    let tot = 0;
    ...
    let eurprev = Number(eurval.toFixed(2));
...
spedisciFattura = async (info, payed, nome) => {
    ...
    const s = dueDate.getDate() + '/' + (dueDate.getMonth() + 1) + '/' + dueDate.getFullYear();
    const o = today.getDate() + '/' + (today.getMonth() + 1) + '/' + today.getFullYear();
    ...
    const toSend = JSON.stringify(fattura);

    console.log(toSend);

    const res = await fetch('https://api.fattureincloud.it/v1/fatture/nuovo', {
        method: 'post',
...
const main = async () => {
    ...
    for (let user of userInfo) {
        ...
        // if (ca == null)
        {
            ...
            // put costanalysis
            // const ca = await documentClient.put({TableName: 'AnalisiCosti',Item: {results}}).promise();
